...
func TestIsRetryableError_ToolExecutionError(t *testing.T) {
	t.Parallel()

	t.Run("tool error wrapping a net.Error is not retryable", func(t *testing.T) {
		t.Parallel()
		...
		if isRetryableError(err) {
			t.Error("expected a tool execution error to not be retryable")
		}
	})

	t.Run("tool error wrapping a retryable ProviderError is not retryable", func(t *testing.T) {
		t.Parallel()
		err := &ToolExecutionError{
			ToolName:   "sub_agent",
			ToolCallID: "call-1",
	...
	t.Run("unwraps to the tool's own error", func(t *testing.T) {
		t.Parallel()
		inner := &net.OpError{Op: "dial", Net: "tcp", Err: errors.New("connection refused")}
		err := error(&ToolExecutionError{ToolName: "fetch", ToolCallID: "call-1", Err: inner})
		var opErr *net.OpError
		if !errors.As(err, &opErr) {
			t.Fatal("expected errors.As to reach the wrapped net.OpError")
		}
		if got, want := err.Error(), `tool "fetch" failed: dial tcp: connection refused`; got != want {
			t.Fatalf("Error() = %q, want %q", got, want)
		}
	})
}

func TestRetryWithExponentialBackoff_ToolExecutionError(t *testing.T) {
	t.Parallel()
	...
	if !errors.Is(err, toolErr) {
		t.Fatalf("expected the tool execution error to be returned as-is, got %v", err)
	}
	var retryErr *RetryError
	if errors.As(err, &retryErr) {
		t.Fatalf("expected no RetryError wrapper on a first-try failure, got %v", err)
	}
	if attempts != 1 {
		t.Fatalf("expected 1 attempt, got %d", attempts)
	}
	if retries != 0 {
		t.Fatalf("expected OnRetry not to be called, got %d calls", retries)
	}
}
